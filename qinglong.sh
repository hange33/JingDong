...
EOF
    ...
    if [ $? -ne 0 ] ; then
        ...
        sleep 30
        ok "青龙面板已启动，请去浏览器访问http://ip:5777进行初始化并登陆进去，完成后回来继续下一步！"
    fi
...
add_repo() {
if [ "$(grep -c "q1hang/jd" $ql_path/data/config/crontab.list)" != 0 ]; then
...
add_repo
sleep 2
ok "已部署完成，2.11.3版本青龙，数据保存路径为$datav，容器名qinglong，访问地址http://ip:5777"
